...
type RouteState = "idle" | "preparing" | "approval" | "confirming" | "handoff" | "success" | "failed";
type Result = { provider: ProviderId; state: RouteState; message?: string; signatures?: string[]; mint?: string; url?: string };
...
export function LaunchWorkspace() {
  ...
  return <main id="launch" className={`workspace theme-${primary.id}`} style={{ "--provider-accent": primary.accent, "--provider-ink": primary.ink } as React.CSSProperties}>
    ...
    {results.length ? <section className="route-results"><div className="section-title"><span>Live launch status</span><small>Every signature is tracked independently</small></div>{results.map((result) => { const provider = providerById[result.provider]; return <article key={result.provider} className={`route-row ${result.state}`}>
      ...
      {result.signatures?.[0] ? <a href={`https://solscan.io/tx/${result.signatures[0]}`} target="_blank" rel="noreferrer">Explorer <ArrowUpRight size={14}/></a> : result.url ? <a href={result.url} target="_blank" rel="noreferrer">Open Bags <ArrowUpRight size={14}/></a> : null}
...
function ProviderOptions({ id, form, update }: { id: ProviderId; form: LaunchForm; update: (key: keyof LaunchForm, value: string) => void }) {
  ...
  if (id === "bags") return <article className="option-card bags-card"><OptionHead id={id}/><p>Anything prefills the official Bags launch form with your token details and artwork. Review the fee mode and first buy on Bags, then approve the launch there. Anything adds no router fee.</p></article>;
  if (id === "raydium") return <article className="option-card raydium-card"><OptionHead id={id}/><div className="option-grid"><Field label="First buy (SOL)" value={form.initialBuy} onChange={(value) => update("initialBuy", value)} placeholder="0"/><Select label="Graduation" value="auto" onChange={() => undefined} options={[["auto", "Match live config"]]}/></div><p>Creates a standard SOL LaunchLab curve through Raydium SDK v2. The migration route is matched to Raydium’s live configuration before your wallet signs.</p></article>;
...
function RouteIcon({ state }: { state: RouteState }) { if (["preparing", "approval", "confirming"].includes(state)) return <LoaderCircle className="spin" size={18}/>; if (state === "handoff") return <ExternalLink size={18}/>; if (state === "success") return <Check size={18}/>; if (state === "failed") return <CircleAlert size={18}/>; return null; }
